...
class SendMailInvoiceReminder{
    ...
    public function go(){
        ...
        $invoices = $inv->getInvoiceByReminderToday();        
        ...
        if(count($invoices) > 0){                        
            ...
            foreach($invoices as $i){
                ...
                $data=array(                        
                    ...
                    'to'        =>  $i['email'],
                    'name'      =>  $i['first_name'],
                    'subject'   =>  "Everyvents - Invoice Reminder ".$i['code']." a/n ".ucwords($i['first_name'])." Event :".$i['event_name'],
                ...
                echo "Sending Invoice Reminder ".$i['code']." to ".$i['email']."...\n";
                ...
                if($mail['status'] == 1){
                    ...
                    DB::update('tr_invoices','code',$i['code'],array(
                        'reminder_date'    =>  date('Y-m-d H:i:s', strtotime(date('Y-m-d'). ' +1 day')),
                        'updated_at'       =>  date('Y-m-d H:i:s'),
                    ));

                    //post to history
                    DB::insert('tr_invoices_reminder_duedate',array(
                        'i_user'        =>  $i['bill_to'],
                        'i_invoice'     =>  $i['code'],
